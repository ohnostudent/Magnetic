...
void LineIntegralConv ::writeBmp(char *filename)
{
  ...
  imageUC[1] = 'M';
  /* File size 4 bytes*/
  val1 = (char)(val & filesize);
  val2 = (char)(val & (filesize >> 8));
  val3 = (char)(val & (filesize >> 16));
  val4 = (char)(val & (filesize >> 24));
  ...
  for (i = 6; i <= 9; i++)
    imageUC[i] = (char)0; /* reserved area*/
  ...
  /* width size */
  val1 = (char)(val & width);
  val2 = (char)(val & (width >> 8));
  val3 = (char)(val & (width >> 16));
  val4 = (char)(val & (width >> 24));
  ...
  /* Height size*/
  val1 = (char)(val & height);
  val2 = (char)(val & (height >> 8));
  val3 = (char)(val & (height >> 16));
  val4 = (char)(val & (height >> 24));
...
void LineIntegralConv ::setStreamParam(double div, int rs)
{
  ...
  rungekutta_step = rs;
  if (debug)
    printf("LineIntegralConv, div, rungekutta max step = %f, %d\n", stream_div, rungekutta_step);
}

void LineIntegralConv ::allocMem()
{
  if (debug)
    printf("allocMem numlicpxl=%d\n", numlicpxl);
  int x = dat_size[0];
  int y = dat_size[1];
  fx = new float[x * y];
  fy = new float[x * y];
...
static float vect_inner_product(float a[], float b[])
{
  return (a[0] * b[0] + a[1] * b[1] + a[2] * b[2]);
}

static double vect_amp(float vec[])
{
  double vx, vy, vz;
  vx = vec[0];
  vy = vec[1];
...
int LineIntegralConv::find_fact(double x, double y, int *i, int *j, double cx[4], double cy[4])
{
  ...
  if (x <= region_min[0] || x >= region_max[0] || y <= region_min[1] || y >= region_max[1])
    return (0);
...
int LineIntegralConv::vec_value(double x, double y, double z, double *vx, double *vy, double *vz, double *vec_abs)
{
  double cx[4], cy[4];
  int i, j;
...
float LineIntegralConv::stream_line(int x0[2])
{
  int i, j, direction;
  float h;
  double x[3];
  ...
  for (direction = 0; direction < 2; direction++)
  {
    ...
    for (j = 1; j < rungekutta_step; j++)
    {
      ...
      for (i = 0; i < n; i++)
      {
        f[n + i] = -c01 * f[2 * n + i] + c03 * f[7 * n + i] + c14 * f[6 * n + i] - c15 * f[4 * n + i] + c16 * f[3 * n + i] + f[i];
      }
      ...
      for (i = 0; i < n; i++)
      {
        f[n + i] = -c18 * f[7 * n + i] + c19 * f[8 * n + i] + c20 * f[2 * n + i] - c21 * f[6 * n + i] - c22 * f[3 * n + i] + c23 * f[5 * n + i] + f[i];
      }
      ...
      for (i = 0; i < n; i++)
      {
        x[i] = f[i] = (f[5 * n + i] + f[7 * n + i]) * c24 + (f[2 * n + i] + f[3 * n + i]) * c25 + (f[4 * n + i] + f[8 * n + i]) * c26 + f[6 * n + i] * c27 + f[i];
      }
      ...
      if (cpxl[0] != prev_x[0] || cpxl[1] != prev_x[1])
      {
        ...
        rnlicpxl[direction]++;
        if (rnlicpxl[direction] > numlicpxl)
          break;
      }
      // End of Calculations
      // project_lines2plane (x, line_color);
  ...
  for (int dir = 0; dir < 2; dir++)
    for (int i = 0; i < rnlicpxl[dir]; i++)
    {
      ...
      pxcolor += (1.0 + cos(PI * i / (numlicpxl - 1.0))) * whitenoise[px + py * pic_w] * (PI / numlicpxl) / 2.0 / PI;
    }
  // printf("color = %f\n",pxcolor);
  return pxcolor;
}
