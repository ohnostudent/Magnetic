...
class _kernel():
    ...
    """
    def testcarnel(self, im1,im2):
        return (im1 + im2) /2
        
    def carnellistxy(self, im1, im2):#xy交互のリストを持った行列を返す。shapeが1次元増えるので使わない
    ...
    def carnelxy(self, im1, im2):#xy交互の行列を返す。shapeのx方向が2倍になる。
    ...
    def carnelEnergy(self, vx, vy, dens):
        return dens * (vx**2 + vy**2) / 2

    def carnelRad(self, vx,vy):
